// Multi-cluster extension of ane-mutable-adapters/harness/swap.mm.
// Identical except e5rtMutableMILWeightURLs is built from N key/URL PAIRS:
//   swap_multi <model> <units> <key1> <url1> [<key2> <url2> ...]
...
int main(int argc,char**argv){@autoreleasepool{ setvbuf(stdout,NULL,_IONBF,0);
  ...
  if(argc>4 && strcmp(argv[3],"none")!=0){
    ...
    for(int i=3;i+1<argc;i+=2) d[@(argv[i])]=[NSURL fileURLWithPath:@(argv[i+1])];
    printf("clusters=%lu  ", (unsigned long)d.count);
  ...
  printf("pred_token=%d  logit=%.4f  sum_abs=%.2f\n",bi,best,sa); return 0;}}
